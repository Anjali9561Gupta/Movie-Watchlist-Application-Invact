...
export const addMovie = (data) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(`http://localhost:5000/movie`, data);
...
export const updateMovieData = (id, data) => async (dispatch) => {
  ...
  try {
    const res = await axios.put(`http://localhost:5000/movie/${id}`, data);
...
export const reviewUpdate = (id, {reviews}) => async (dispatch) => {
    ...
    try {
      const res = await axios.patch(`http://localhost:5000/movie/${id}/review`, {reviews});
...
export const deleteMovie = (id) => async (dispatch) => {
  try {
    let res = await axios.delete(`http://localhost:5000/movie/${id}`);
...
export const toggleWatchStatus = (id, watchStatus) => async (dispatch) => {
  try {
    const res = await axios.patch(
      `http://localhost:5000/movie/${id}/watchStatus`,
      { watchStatus }
    );
